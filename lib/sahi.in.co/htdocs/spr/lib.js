...
Sahi.prototype.toJSON2 = function(el, map){
	...
	if (el instanceof Date) el = el.toString(); // needs to quoted later	
...
Sahi.prototype.setStep = function(cmd, debugInfo, stepType){
    //this.print(cmd);
    return ScriptRunner.setStep(cmd, debugInfo, stepType, this.applicationType);
};
Sahi.prototype.executeWait = function(cmd, debugInfo){
    ...
    if (this.isBrowser()) {
		...
		this.lastId = this.setStep(cmd, debugInfo, "WAIT");
...
Sahi.prototype.executeWithLogging  = function(cmd, debugInfo, stepType){
	try {
		...
		if (e.debugInfo == "FORCED_STOP_TEST_CASE") {
			ScriptRunner.stripAndLog(cmd, debugInfo, "SUCCESS", e.message);
		//} else if (e.debugInfo == "FORCED_FAIL") {
...
Sahi.prototype.sL = function (cmd, debugInfo, e) {
	if (e) {
		if(e.debugInfo == "FORCED_STOP_TEST_CASE") {
			ScriptRunner.stripAndLog(cmd, debugInfo, "SUCCESS", (e && e.message) ? e.message : e);
		} else {
		...
		if(!this.isScreenShot(cmd)){
			var stepType = "INFO";
			if(this.disableInfo && cmd.indexOf("_disableInfoLogging") != 0) stepType = "NO_LOG";
			ScriptRunner.stripAndLog(cmd, debugInfo, stepType, "");
		}
	}
}
Sahi.prototype._stopTestCase = function (message) {
	throw new SahiException("Stop" + (message ? (": " + message) : ""), "FORCED_STOP_TEST_CASE");
}
...
Sahi.prototype.schedule2 = function(cmd, debugInfo, cycles, stepType, throwException){
    ...
    if (cmd.indexOf("_sahi._assert") == 0 && !this.isBrowser()) {
    	this.evalAssertInLib(cmd, debugInfo, stepType);
    	return;
    }
    ...
    var i=0;
    var tempStatus;
    while(ScriptRunner.isPaused() || i++ < cycles){
...
Sahi.prototype.evalAssertInLib = function (cmd, debugInfo, stepType) {
	try {
		eval (cmd);
		this.lastId = this.setStep(cmd, debugInfo, stepType);
		ScriptRunner.markStepDoneFromLib(""+this.lastId, "success", null);
	} catch (e) {    	
		this.lastId = this.setStep(cmd, debugInfo, stepType);
		ScriptRunner.markStepDoneFromLib(""+this.lastId, "failure", e.toString());
	}	
}
...
//	}
    ...
    while(i++ < cycles){
        if (this.firstTime_FOR_TESTING_ONLY || !ScriptRunner.isRunning()) {
            ...
            if (this.isNoise()) return;
            var globalScript = this.getGlobalScript();
            if (globalScript && new java.io.File(globalScript).exists()) {
...
Sahi.prototype.getGlobalScript = function () {
	var globalScript = null;
	var isDrun = this.isDrun();
	var isDload = this.isDload();
	var isNonDistributedRun = this.isNonDistributedRun();
	if((isDrun || isDload) && !isNonDistributedRun){
		var copiedScriptsPath = this._suiteInfo()['copiedScriptsPath'];
		globalScript = net.sf.sahi.util.Utils.concatPaths(net.sf.sahi.config.Configuration.getUserDataDir(), copiedScriptsPath, 
    			net.sf.sahi.config.Configuration.getGlobalncludeFile());
	}
	else{
		var suitePath = this._suiteInfo()['suitePath'];
		var scriptRoot = net.sf.sahi.config.Configuration.getScriptRootFromSuitePath(suitePath);
		if(scriptRoot == "") return null;
		globalScript = net.sf.sahi.util.Utils.concatPaths(scriptRoot, 
    			net.sf.sahi.config.Configuration.getGlobalncludeFile());
	}
	return globalScript;
};
Sahi.prototype.isDrun = function () {
	var nodes = this._suiteInfo()['nodes'];
	if(nodes) return true;
	return false;
};
Sahi.prototype.isNonDistributedRun = function () {
	var isNonDistributedRun = this._suiteInfo()['isNonDistributedRunS'];
	if(isNonDistributedRun == "true") return true;
	return false;
};
Sahi.prototype.isNoise = function () {
	var isNoise = this._suiteInfo()['isNoise'];
	if(isNoise == "true") return true;
	return false;
};
Sahi.prototype.isDload = function () {
	var isDload = this._suiteInfo()['isLoadS'];
	if(isDload == "true" || this.isNoise()) return true;
	return false;
};
...
Sahi.prototype._takeScreenShot = function(fileSysPath, noLog, props) {
	...
	if(!noLog){
		...
		ScriptRunner.getReport().setScreenShot(stepId, fileName, null);
	}
	else{
...
Sahi.prototype._windowAction = function(s, width, height){
	...
	if(Packages.net.sf.sahi.util.Utils.isWindows()){
		...
		if(width || height){
			var cmd = "cmd /C " + this.quoted(("" + net.sf.sahi.config.Configuration.getAbsolutePath("tools/winAction.exe")).replace('/', '\\')) + " " + this.quoted(s)+" "+ this.quoted(newTitle) + " " + width +  " " + height;
		}
		else
			var cmd = "cmd /C " + this.quoted(("" + net.sf.sahi.config.Configuration.getAbsolutePath("tools/winAction.exe")).replace('/', '\\')) + " " + this.quoted(s)+" "+ this.quoted(newTitle);
		this._execute(cmd, true);
	...
	else if(Packages.net.sf.sahi.util.Utils.isLinux() && Packages.net.sf.sahi.config.Configuration.getWindowActionEnabledInLinux()) {
		var oldTitle = _sahi._title() || "";
		var newTitle = ""+(new Date()).getTime();
		this.scheduleNoLog("window.document.title = " + this.quoted(newTitle));
		this.wait(net.sf.sahi.config.Configuration.getWindowActionWaitTime("window_action_api.wait_after_unique_title_set"));
		...
		var key;
		if(s == "focus") key = "windowactivate --sync";
		else if(s == "maximize") key = "windowsize --sync 100% 100%";
		else if(s == "minimize") key = "windowminimize --sync";
		else if(s == "restore") key = "windowmap --sync";
		else if(s == "refresh") key = "windowactivate --sync key --clearmodifiers ctrl+r";
		else if(s == "resize") key = "windowsize --sync " + width + " " + height;
		var cmd = "xdotool search --name " + this.quoted(newTitle) + " " + key;	
		this._execute(cmd, true);
...
Sahi.prototype._debugToFile = Sahi.prototype._writeFile;
...
Sahi.prototype._setSpeed = function(ms){
	...
	ScriptRunner.getSession().setTimeBetweenSteps(ms);
}
Sahi.prototype.makeAssociative = function(array2d){
	array2d.set = function(rowHeader, colHeader, newValue){
        ...
        if ((typeof colHeader) == "number") {
        	if (colHeader < 0 || colHeader >= this[0].length) {
        		return null;
        	}
        } else {
        	for (var i=this[0].length; i>=0; i--) {
    			if (colHeader == this[0][i]) {
    				colHeader = i;
    				break;
    			}
    		}
        }
        this[rowHeader][colHeader] = newValue;
    }
	array2d.get = function(rowHeader, colHeader){
        ...
        return this[rowHeader][colHeader];
    }
	
	array2d.filter = function(groupName, headerAr) {
		var rs = [];
		var isGroupName = false;
		for(var i=0; i<this.length; i++){
			var row = this[i];
			if(_sahi.arrayNullOrBlank(row)) {
				continue;
			}
			if (groupName == undefined || groupName == null || groupName == "") {
				...
				var value = row[0];
				if (!isGroupName && value == groupName) {
					rs.push(row.slice(1, row.length));
					isGroupName = true;
				}
				else if (isGroupName && (value == undefined || value == null || value == "" || value == "null")) {
					rs.push(row.slice(1, row.length));
				}
				else if (rs.length > 0) {
					break;
		...
		_sahi.associativeArray(rs, true);
		var ar = [];
		if (headerAr && (headerAr instanceof Array) && headerAr.length > 0) {
			for (var j=0; j<rs.length; j++) {
				ar[j] = [];
				var r = rs[j];
				for (var k=0; k<headerAr.length; k++) {
					var val = rs.get(j, headerAr[k]);
					if (val === undefined) val = null;
					ar[j].push(val);
				}
			}
			return _sahi.associativeArray(ar, true);
		}
		return rs;
	}
...
Sahi.prototype.associativeArray = function(array2d, includeHeader){
	...
	for(var i = 0; i < array2d.length; i++){
		for(var j=0; j < header.length; j++){
			if (header[j] && header[j].match(/^[0-9]+$/)) continue;
...
Sahi.prototype._getGlobal = function(key){
	var suiteVariables = ScriptRunner.getSession().getSuiteVariables();
	var val = suiteVariables.getVariable(key);
	return eval('('+val+')');
}
...
Sahi.prototype._scriptPath = function(fileSeparator){
	var path = ("" + ScriptRunner.getCurrentIncludePath()).replace(/\\/g, "/");
	if (fileSeparator && fileSeparator == "\\") path = path.replace(/\//g, "\\");
    return path;
};
...
Sahi.prototype._sessionInfo = function(){
    var info = eval("(" + ScriptRunner.getSession().getInfoJSONForLib() + ")");
...
Sahi.prototype._suiteInfo = function(){
	...
	try{
		var suiteInfoJson = suiteInfo.toJSON();
		var info = eval("(" + suiteInfoJson + ")");
	}catch(e){
		this.print(suiteInfoJson);
		this.logExceptionAsError(e);
	}
    return info;
};

Sahi.prototype.areTagsMatching = function(tags){
	var match = ScriptRunner.areTagsMatching(tags);
    return  eval("(" + match + ")");
};

Sahi.prototype.getTestcasesToRun = function(){
	return ScriptRunner.getTestcasesToRun();
};
...
//	this.print(data);
	...
	return this.makeAssociative(dataOut);
};
...
Sahi.prototype._resolvePath = function(path, fileSeparator){
	if (this.currentScriptPath == null) this.setCurrentScriptPath(this._scriptPath()); 
	var thisPath = this.currentScriptPath.replace(/\\/g, "/");
	var path = ("" + net.sf.sahi.util.Utils.concatPaths(thisPath, path)).replace(/\\/g, "/");
	if (fileSeparator && fileSeparator == "\\") path = path.replace(/\//g, "\\");
    return path;
}
Sahi.CSV_NEWLINE = "\r\n";
Sahi.prototype._writeCSVFile = function(array2d, filePath, overwrite, separator, forceQuotes) {
	filePath = this._resolvePath(filePath);
	if (!separator) separator = ",";
	if (forceQuotes == null) forceQuotes = true;
	var s = [];
	for (var i=0; i < array2d.length; i++) {
		var row = array2d[i];
		for (var j=0; j < row.length; j++) {
			...
			if(forceQuotes){
				...
				if(cell.indexOf(separator) != -1) s[s.length] = this.escapeForCSV(cell);
				else s[s.length] = cell;
			}
...
Sahi.prototype._getExcel = function(fileName,sheetName){
	...
	return new ExcelPOIWrapper(new Packages.net.sf.sahi.util.ExcelPOI(fileName,sheetName), this);
};
...
Sahi.prototype._fileExists = function (filePath) {
	filePath = this._resolvePath(filePath);
	return this.doesFileExist(filePath);
};

Sahi.prototype._readExcelFile = function(fileName, sheetName, includeHeader, ignoreEmptyRows){
	fileName = this._resolvePath(fileName);
	//if (!sheetName) sheetName = "Sheet1";
	if (!ignoreEmptyRows) ignoreEmptyRows = false;
	var poi=new Packages.net.sf.sahi.util.ExcelPOI(fileName,sheetName);
	var javaAr = poi.getData(true, ignoreEmptyRows);
...
Sahi.prototype._userDataDir = function(fileSeparator){
	var path = ("" + net.sf.sahi.config.Configuration.getUserDataDir()).replace(/\\/g, "/");
	if (fileSeparator && fileSeparator == "\\") path = path.replace(/\//g, "\\");
    return path;
}
Sahi.prototype._userDataPath = function(relPath, fileSeparator){
	var path = ("" + net.sf.sahi.config.Configuration.getAbsoluteUserPath(relPath)).replace(/\\/g, "/");
	if (fileSeparator && fileSeparator == "\\") path = path.replace(/\//g, "\\");
    return path;
}
Sahi.prototype._isDataPassed = function(){
	if(typeof $isDataPassed == "boolean") return $isDataPassed;
...
Sahi.prototype._launchNewBrowser = function(url, browserType) {
	try {
		if (typeof url == "undefined" && typeof browserType == "undefined") {
			return ScriptRunner.launchNewBrowser();
		} else if (typeof browserType == "undefined") {
			return ScriptRunner.launchNewBrowser(url);
		} else {
			return ScriptRunner.launchNewBrowser(url, browserType);
		}
	} catch (e) {
...
Sahi.prototype.fnStart = function(fnName, o, args) {
	if ((fnName.indexOf("SahiFrameWork")==0)  || (o && args && o instanceof args.callee) || args.callee.noLog) return null; 
...
Sahi.prototype._dataDrive = function(func, array2D, preFn, postFn){
	for (var i=0; i<array2D.length; i++){
		var args = array2D[i];
		if(args.length > 0 && ("" + args[0]).indexOf("//") == 0) continue;
		if (preFn) preFn();
		try{
...
Sahi.prototype.makeFetchAPIs = function(){
	var apis = ["_getValue", "_getAttribute", "_containsText", "_getTableContents","_getScreenSize",
	            ...
	            "_lastDownloadedFileName", "_prompt", "_confirm", "_count", "_extract", "_getSelectionText", "_collectAttributes", "_areEqual", "_collectElementsInfo"];
...
Sahi.prototype._positionNative = function(el){
	return this.placePatternAndGetElementPositionRelativeToScreen(el);
}
...
/* Native end */
Sahi.prototype._resource = function(src){
...
Sahi.prototype.setCurrentScriptPath = function(path) {
	this.currentScriptPath = path;
	if (path == undefined) path = null;
	ScriptRunner.setCurrentScriptPath(path);
}

Sahi.prototype._testcasesInfo = function (){
	var summaries = ScriptRunner.getCurrentTestCaseSummary();
	var json = [];
	for(var i=0; i<summaries.size(); i++){
		var summary = summaries.get(i);
		var status = summary.isFailed()? "FAILURE" : "SUCCESS";
		var id = summary.getId();
		var description = summary.getDescription();
		json.push({id: id, description: description, status: status});
	}
	return json;
}

/* --------- MSWindows Start ---------- */
Sahi.prototype._setMode = function (applicationType) {
	this._selectWindow();
	this._selectDomain();
	this.applicationType = applicationType;
}
Sahi.prototype.isBrowser = function() {
	return this.applicationType == null || this.applicationType.toLowerCase() == "browser";
}
...
RESTResponse.prototype.getBodyAsString = function(charSet){
	if (charSet == null) charSet = "UTF-8";
	return "" + new java.lang.String(this.$response.data(), charSet);
}
//----------------DUPLICATED FROM CONCAT.JS-----------------
Sahi.prototype._assertEqual = function (expected, actual, s) {
	if ((this.isArray(expected) && this.isArray(actual)) || (this.isObject(expected) && this.isObject(actual))){
		var compareResult = this.compare(expected,actual);
		if (compareResult != "equal") {
			var msg = compareResult.errorMsg + " at ";
			for ( var i = compareResult.stack.length - 1; i >= 0; i--) {
				msg += compareResult.stack[i];
			}
			compareResult = msg;
		}
		if (compareResult != "equal") throw new SahiAssertionException(3,(s ? s : "") + "\n"+compareResult);
		return true;
	}else if (!this.areEqualParams(this.trim(actual), this.checkRegex(this.trim(expected)))) 
...
Sahi.prototype.isObject = function (obj) {
	return (obj instanceof Object);
}
Sahi.prototype.compare = function (a1, a2) {
	var result = "equal";
	if(a1==a2) return result;
	if (a1 instanceof Array && a2 instanceof Array) {
		result = this.compareArrays(a1, a2);
	} else if (a1 instanceof Object && a2 instanceof Object) {
		result = this.compareObjects(a1, a2);
	} else if (!this.areEqualParams(this.trim(a2), this.checkRegex(this.trim(a1)))) {
		return {errorMsg:"Expected: " + this.toJSON(a1) + "\nActual: " + this.toJSON(a2), stack:[]};
	}
	return result;
}
Sahi.prototype.compareArrays = function (a1, a2) {
	if (a1 == null && a2 == null) return "Both the arrays are null";
	if (a1 == null || a2 == null) return "One of the arrays is null";
	if (a1.length != a2.length && typeof(a1) == typeof(a2)) return {errorMsg:"Difference in length of arrays:\nExpected Length:["+a1.length+"]\nActual Length:["+a2.length+"]", stack:[]};
	var result = "equal";
	for(var i=0; i<a1.length; i++){
		result = this.compare(a1[i], a2[i])
		if (result.errorMsg){
			result.stack.push("["+i+"]");
			return result;
		}
...
Sahi.prototype.compareObjects = function (a1, a2) {
	var propcount1 = 0;
	var propcount2 = 0;
	var result = {};
	for (var propertyName in a1) {
		if (!a2.hasOwnProperty(propertyName)) {
			return {errorMsg:"Expected property '" + propertyName + "' does not exist.", stack:[]};
		}
		result = this.compare(a1[propertyName], a2[propertyName]);
		if (result.errorMsg){
			result.stack.push("["+this.quoted(propertyName)+"]");
			return result;
		}
		propcount1++;
	}
	if (propcount1 > 0) {
		for(var propertyName in a2) {
			propcount2++;
		}
		if(propcount1 !=propcount2) {
			return {errorMsg:"Difference in length of objects:\nExpected Length: "+propcount1+"\nActual Length: " + propcount2, stack:[]};
		}
	}
	return result;
}
Sahi.prototype.areEqualParams = function(actual, input){
	...
	if (this.isAccessorIgnoreCase) {
		return (this.lowerCase(actual) == this.lowerCase(input));        	
	} else {        	
		return (actual == input);
	}
}
Sahi.prototype.checkRegex = function(s, makeRegexGlobal, ignoreCase){
	try{
		return ((typeof s) == "string" && s.match(this.CHECK_REGEXP)) ?  eval(s + ((!s.match(/\/i$/) && ignoreCase)?"i":"") + (makeRegexGlobal?"g":"")) : s;
	}
	catch(e) {
		return s;
	}
...
Sahi.prototype._assertNotNull = function (n, s) {
	...
	if (!this._exists(n)) throw new SahiAssertionException(1, s);
	return true;
};
Sahi.prototype._assertExists = Sahi.prototype._assertNotNull;
Sahi.prototype._assertNull = function (n, s) {
	if (this._exists(n)) throw new SahiAssertionException(2, s);
};
...
Sahi.prototype._assertVisible = function (el , s) {
	if(this._isVisible(el)!=true) throw new SahiAssertionException(5,s);
	return true;
}
Sahi.prototype._assertNotVisible = function (el , s){
	if(this._isVisible(el)!=false) throw new SahiAssertionException(5,s);
	return true;
	
}
Sahi.prototype._assertFalse = Sahi.prototype._assertNotTrue;
//ExcelWrapper start
function ExcelPOIWrapper(poi, sahi){
	this.poi = poi;
	this.sahi = sahi;
};
ExcelPOIWrapper.prototype.getDataForDataDrive = function() {
	var includeHeader = true; // NOTE: TRUE
	var ignoreEmptyRows = false;
	var javaAr = this.poi.getData(true, ignoreEmptyRows);
	return this.sahi.associativeArray(this.sahi.convertJavaArrayToJS(javaAr), includeHeader);
}
ExcelPOIWrapper.prototype.__noSuchMethod__ = function(fnName, args){
	return this.poi[fnName].apply(this.poi, args);
};
// ExcelWrapper end


Sahi.prototype.initMultiLanguage = function() {
	this.toLangMap = null;
	this.reversedMap = null;
	this.isLanguageSet = false;
	this.areLanguagesSame = false;
}

Sahi.prototype._setLanguage = function(toLangFile, fromLangFile, append) {
	this.isLanguageSet = true;
	toLangFile = this._resolvePath(toLangFile);
	fromLangFile = this._resolvePath(fromLangFile);
	if (append && this.toLangMap != null) {
		this.toLangMap.putAll(Packages.net.sf.sahi.util.Utils.loadPropertiesUTF8(toLangFile));
	} else {
		this.toLangMap = Packages.net.sf.sahi.util.Utils.loadPropertiesUTF8(toLangFile);
	}
	if (toLangFile == fromLangFile) {
		this.areLanguagesSame = true;
		return;
	}
	if (append && this.reversedMap != null) {
		this.reversedMap.putAll(Packages.net.sf.sahi.util.Utils.loadReversedPropertiesUTF8(fromLangFile));
	} else {
		this.reversedMap = Packages.net.sf.sahi.util.Utils.loadReversedPropertiesUTF8(fromLangFile);
	}
};
Sahi.prototype.getTranslation = function(key, defaultVal) {
	if (this.toLangMap.containsKey(key)) {
		return this.toLangMap.get(key);
	} else {
		return defaultVal;
	}
}
function $sahi_translate(s) {
	if (!_sahi.isLanguageSet) return s;
	if (s.indexOf("L:") == 0) {
		return s.substring(2);
	}
	else if (s.indexOf("K:") == 0) {
		var key = s.substring(2);
		return _sahi.getTranslation(key, s);
	}
	else {
		if (_sahi.areLanguagesSame) return s;
		var key = _sahi.reversedMap.get(s);
		if (key == null) return s;
		return _sahi.getTranslation(key, s);
	}
}

Sahi.prototype._setPingDelayAfterStep = function(t){
	ScriptRunner.getSession().setPingDelayAfterStep(t);
}
