...
#TODO  supprimer user_agent, nationality car sera utilisé celui du poste hébergeur (virtuel ou physique)
#TODO supprimer les customisation des query http en sortie du navugateur car Firefox n'est plus utilisé pour se substituer aux autres navigateurs, mais pour lui -même
module Browsers
  module Webdriver
    require 'json'
    class CustomQuery
    ...
    class Browser
      ...
      EXTENSION_CATCH_REQUEST = "catchrequest@statupbot.com"
      attr :driver,
           :screen_resolution
      ...
      def initialize(browser_details, user_agent)
        ...
        case RbConfig::CONFIG['host_os']
          when /mswin|msys|mingw|cygwin|bccwin|wince|emc/
            ...
            @profile["extensions.#{EXTENSION_CATCH_REQUEST}.log.filename"] = File.join(File.absolute_path(LOG_DIR), "#{EXTENSION_CATCH_REQUEST}.log").gsub("/", "\\")
          when /linux/
        ...
        @profile["extensions.#{EXTENSION_CATCH_REQUEST}.debugging"] = $debugging
        @profile["extensions.checkCompatibility"] = false
        #  @profile.add_extension(File.join(EXTENSION_DIR, EXTENSION_URL_REWRITING))
        @profile.add_extension(File.join(EXTENSION_DIR, EXTENSION_CATCH_REQUEST))
