...
#----------------------------------------------------------------------------------------------------------------
    ...
    # landing_page
    #----------------------------------------------------------------------------------------------------------------
    # retourn la premier page de la visite sur laquelle point le referer
    #----------------------------------------------------------------------------------------------------------------

    def self.landing_page(pages)
      #TODO s'assurer que les pages sont ordonnées par heure => faire un tri
      pages[0].url
    end
    ...
    def initialize(page, start_date_time, visit_id)
      ...
      @start_date_time = start_date_time + page["delay_from_start"].to_i + 60 #TODO à supprimer
...
#----------------------------------------------------------------------------------------------------------------
    ...
    def browse()
      ...
      if visit.nil?
        raise PageException, "#{@visit.id} not found"
      else
        @visit =visit
      end
      begin
