...
class Hash
  ...
  def to_html_stat
    [
        '<ul>',
        map { |k, v| [!k.is_a?(Date) ? "<li><strong>#{k}</strong>"  : "<li><strong>#{k}</strong> : ", v.respond_to?(:to_html) ? v.to_html_stat : " (<strong>#{v}</strong>)</li>"] },
        '</ul>'
    ].join
  end
  ...
  def to_html
    [
        '<ul>',
        map { |k, v| ["<li>#{display_code(k, v)}", v.respond_to?(:to_html) ? v.to_html : " #{v}</li>"] },
        '</ul>'
    ].join
  end

  def display_code(k, v)
                        ...
                        <HTML>
                         <HEAD>
                          <BODY>
                            ...
                            #{@return_codes_stat.to_html_stat}</li>
                              <li><h1>Errors history</h1>
                            #{@return_codes.to_html}</li>
                            </ul>
                          <BODY>
                        </HEAD>
                        </HTML>
